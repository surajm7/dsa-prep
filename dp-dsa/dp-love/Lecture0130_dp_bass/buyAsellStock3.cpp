    ...
    int solve(int index,int buy,vector<int>&prices,int limit){
      ...
      int solveMem(int index,int buy,vector<int>&prices,int limit,vector<vector<vector<int>>> &dp){
        ...
        if(buy){
            ...
            int skipIt =0+solveMem(index+1,1,prices,limit,dp);
            profit=max(buyIt,skipIt);
        }else{
             int sellIt=prices[index]+solveMem(index+1,1,prices,limit-1,dp);
            int skipIt =0+solveMem(index+1,0,prices,limit,dp);
            profit=max(sellIt,skipIt);
            ...
            for(int buy=0;buy<=1;buy++){
                for(int limit=1;limit<=2;limit++){
                    ...
                    if(buy){
                        ...
                        int skipIt =0+dp[index+1][1][limit];
                        profit=max(buyIt,skipIt);
                    }else{
                        int sellIt=prices[index]+dp[index+1][1][limit-1];
                        int skipIt =0+dp[index+1][0][limit];
                        profit=max(sellIt,skipIt);
                    }
